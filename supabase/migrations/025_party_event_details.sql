-- 025: Persist event identity for arbitrary future football games.
--
-- Team names alone are not enough context for reusable pools. A host should be
-- able to name the event ("2027 Super Bowl", "Thanksgiving Game") and, when
-- known, attach kickoff time so shared links and recents remain recognizable.

ALTER TABLE parties
  ADD COLUMN event_name VARCHAR(80) NOT NULL DEFAULT 'Football Squares',
  ADD COLUMN kickoff_at TIMESTAMPTZ;

DROP FUNCTION create_party(
