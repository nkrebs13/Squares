...
The screenshots in `docs/screenshots/` are manual captures — they're not generated by the build or test suite. Update them when the UI changes meaningfully.

### Screenshots (hero.png, filling.png, active.png, complete.png)
...
```bash
# 1. Record the walkthrough (create → claim → lock → score → winners)
#    Save the raw recording outside the repo

# 2. Convert to an optimized GIF (target ≤ 2 MB)
