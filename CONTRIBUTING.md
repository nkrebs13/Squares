...
By contributing, you agree to uphold our [Code of Conduct](CODE_OF_CONDUCT.md). First time? See the [Quick start in README.md](README.md#quick-start) to get the app running locally.
...
5. `e2e-tests` — Playwright against a preview server (specs use mocked Supabase routes; see [docs/E2E_TESTING.md](docs/E2E_TESTING.md))

A PR can't merge until all five are green (see [`docs/BRANCH_PROTECTION.md`](docs/BRANCH_PROTECTION.md) for the required-check setup).
...
- **Game-day operations:** [GAME-DAY.md](docs/GAME-DAY.md)
...
The screenshots in `docs/screenshots/` are not generated by the build or test suite. Update them when the UI changes meaningfully.

### Automated (recommended)

`scripts/capture-screenshots.ts` creates a temporary party via RPC, populates it with demo players, advances through all game states, captures every screenshot, generates the demo GIF, and cleans up — all in one run.

```bash
supabase start             # local Supabase must be running
npm run build && npm run preview
npm run screenshots        # runs scripts/capture-screenshots.ts
```

Requires `VITE_SUPABASE_URL` and `VITE_SUPABASE_ANON_KEY` (and optionally `SUPABASE_SERVICE_KEY`) in `.env` or `.env.local`. Also requires `ffmpeg` on your PATH for GIF generation.

### Manual fallback

```bash
supabase start
supabase db reset          # seeds DEMO01 party
npm run build && npm run preview
# Open http://localhost:4173/party/DEMO01 (PIN: 0000)
# Capture filling state (default after reset)
# Use the admin panel to lock the grid → capture active state
# Enter a final score → capture complete state
# Save as docs/screenshots/hero.png, filling.png, active.png, complete.png
supabase stop
```
...
Target ≤ 2 MB. Drop fps to 10 or scale to 640 if it exceeds that.
